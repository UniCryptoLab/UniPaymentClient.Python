...
class ExchangeAPI(BaseClient):
    ...
    def accept_quote(self, access_token, quote_id) -> AcceptQuoteResponse:
        ...
        response_text = self.call_api(url, 'PUT', access_token)
        return AcceptQuoteResponse.from_json(response_text)

    def query_exchange_orders(self, access_token,
        ...
        response_text = self.call_api(url, 'GET', access_token, query_params=query_exchange_orders_request.to_str())
        return QueryExchangeOrdersResponse.from_json(response_text)

    def query_exchange_order_by_order_id(self, access_token, exchange_order_id) -> QueryExchangeOrderResponse:
